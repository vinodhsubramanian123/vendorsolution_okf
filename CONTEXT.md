...
- **Observability & Telemetry Standards**: Enforced strict `logging` instantiation across all LangGraph process nodes (`ikp.workflow.nodes`) to ensure complete telemetry. Every state transition and node execution must be observable to track the agent's reasoning loops and portal rejections. Integrated Langfuse via `@telemetry_trace` for deep LLM pipeline tracing.
...
- **Multi-Key Gemini Pool**: Created `KeyManager` to support rotating through multiple comma-separated free-tier Google AI Studio API keys (e.g. `GEMINI_API_KEYS`). Implemented `@retry_with_key_rotation` which intercepts HTTP 429 quota exhaustion errors and seamlessly fails over to the next key without failing the task.
- **Pydantic Structured Outputs**: Migrated intent parsing and component selection to use native Pydantic `response_schema` models with Gemini 2.0. This guarantees 100% deterministic output types and eliminates brittle markdown regex stripping.
