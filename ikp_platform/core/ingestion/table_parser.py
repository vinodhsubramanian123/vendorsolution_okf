...
class TableParser:
    ...
    def parse_document(self, pdf_path: str) -> List[Dict[str, Any]]:
                    ...
                    for c_row in cleaned_table[1:]:
                        for cell_idx, cell_text in enumerate(c_row):
                            ...
                            if sku_match:
                                ...
                                for p in [
                                    "Select the associated trigger SKU",
                                    "If the HPE",
                                    "•",
                                    "−",
                                    "-",
                                ...
                                if len(description) < 30:
                                    ...
                                    best_desc = (
                                        prev_desc
                                        if len(prev_desc) > len(next_desc)
                                        else next_desc
                                    )
                                    if best_desc:
                                ...
                                role = (
                                    "dependency_note"
                                    if self.sku_pattern.search(description)
                                    else "component"
                                )
                                ...
                                if (
                                    ...
                                    qty_match = re.search(
                                        r"(\d+)\s+default", row_text, re.IGNORECASE
                                    )
                                    if qty_match:
        ...
        logger.info(
            f"Extracted {len(extracted_components)} structured components from tables and text."
        )
