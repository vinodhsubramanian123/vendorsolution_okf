...
class HPEQuickSpecsAdapter(BasePDFAdapter):
    ...
    def can_handle(self, text: str) -> bool:
        return "HPE" in text[:5000] and "QuickSpecs" in text[:5000]
    ...
    def extract_components(self, text: str, platform: Platform, structured_components: List[dict]) -> Tuple[List[BaseEngineeringObject], List[DeltaChange]]:
        ...
        for wl in workloads:
            self.delta_changes.append(
                DeltaChange(change_type=DeltaChangeType.NEW_OBJECT, object_id=wl.id, new_value=wl.title)
            )

        processors = self._extract_processors(text, platform)
        self.extracted_objects.extend(processors)

        memory_specs = self._extract_memory(text, platform)
        self.extracted_objects.extend(memory_specs)

        storage = self._extract_storage(text, platform)
        self.extracted_objects.extend(storage)

        networking = self._extract_networking(text, platform)
        self.extracted_objects.extend(networking)

        gpus = self._extract_gpus(text, platform)
        self.extracted_objects.extend(gpus)

        rules = self._extract_rules(text, platform)
    ...
    def _post_process_limits(self, platform: Optional[Platform]):
        ...
        for obj in self.extracted_objects:
            if obj.type == EngineeringObjectType.COMPONENT:
                ...
                if "ocp" in title_desc and (
                    "slot" in title_desc or "enablement" in title_desc
                ):
                    ocp_slots_found += 1
    ...
    def _human_in_the_loop_fallback(self, text: str) -> Platform:
        ...
        """
        logger.warning(
            "Automated Platform Identity Extraction Failed or Confidence < 80%. Triggering HITL fallback."
        )
    ...
    def _infer_solution_domain(self, text: str) -> str:
        ...
        compute_score = (
            text.count("processor")
            + text.count("memory")
            + text.count("dimm")
            + text.count("server")
            + text.count("proliant") * 10
            + text.count("compute") * 5
        )
        storage_score = (
            text.count("storage")
            + text.count("nvme")
            + text.count("ssd")
            + text.count("drive")
            + text.count("alletra") * 10
        )
        networking_score = (
            text.count("switch")
            + text.count("transceiver")
            + text.count("port")
            + text.count("bandwidth")
            + text.count("aruba") * 10
        )
            ...
            if match:
                ...
                if len(candidate) > 5:
                    ...
                    vendor_match = re.search(
                        r"\b(HPE|Dell|Cisco|Lenovo|IBM|Supermicro)\b",
                        title,
                        re.IGNORECASE,
                    )
                    if vendor_match:
                    ...
                    gen_match = re.search(
                        r"\b(Gen\s*\d+|G\d+|v\d+)\b", title, re.IGNORECASE
                    )
                    if not gen_match:
                        # Fallback: scan the first 500 chars for generation if not in title
                        gen_match = re.search(
                            r"\b(Gen\s*\d+|G\d+|v\d+)\b", text[:500], re.IGNORECASE
                        )
                    ...
                    parts = [
                        ...
                        if p.upper() != vendor.upper()
                        and not re.match(r"(?i)^(Gen\d+|G\d+|v\d+|Compute)$", p)
                    ]
                    if len(parts) >= 1:
                        family = parts[0]
                    if len(parts) >= 2:
        ...
        platform = Platform(
            ...
            evidence=[
                EvidenceRecord(
                    ...
                    description="Automated generic extraction",
    ...
    def _extract_workloads_and_requirements(
        ...
        for idx, wl_name in enumerate(extracted):
            wl_id = (
                f"workload-{wl_name.lower().replace(' and ', '-').replace(' ', '-')}"
            )
            wl = Workload(
                id=wl_id,
                title=wl_name,
                ...
                evidence=[
                    EvidenceRecord(
                        source_id=self.source.source_id,
                        confidence=ConfidenceLevel.HIGH,
                        description="Extracted workload from platform overview",
                    )
                ],
            )
            workloads.append(wl)
    ...
    def _extract_processors(
        ...
        for match in proc_pattern.finditer(text):
            ...
            proc = Component(
                ...
                lifecycle_status=LifecycleStatus.ACTIVE,
                capabilities=[core_type, "DDR5", "PCIe Gen5"],
                attributes=[
                    EngineeringAttribute(
                        name="Base Speed", value=float(base_speed), unit="GHz"
                    ),
                    ...
                    EngineeringAttribute(
                        name="DDR5 Speed", value=int(ddr5), unit="MT/s"
                    ),
                ...
                evidence=[
                    EvidenceRecord(
                        source_id=self.source.source_id,
                        confidence=ConfidenceLevel.HIGH,
                        description="QuickSpecs processor table",
    ...
    def _extract_memory(
        ...
        max_mem_match = re.search(
            r"up\s+to\s+(\d+)\s*(?:TB|GB)\s+(?:of\s+)?memory", text, re.IGNORECASE
        )
        if max_mem_match:
        ...
        ddr_match = re.search(
            r"DDR(\d)\s+(?:speed|memory|RDIMM|LRDIMM)", text, re.IGNORECASE
        )
        if ddr_match:
    ...
    def _extract_gpus(self, text: str, platform: Optional[Platform]) -> List[Component]:
        ...
        for pattern in gpu_patterns:
            for match in re.finditer(pattern, text):
                ...
                gpu_id = (
                    f"{platform_id}/components/gpu-{model.lower().replace(' ', '-')}"
                )
                gpu = Component(
                    id=gpu_id,
                    ...
                    attributes=[
                        EngineeringAttribute(
                            name="GPU Memory", value=int(memory), unit="GB"
                        ),
                    ],
                    relationships=[
                        EngineeringRelationship(
    ...
    def _extract_rules(self, text: str, platform: Optional[Platform]) -> List[Rule]:
        ...
        for pattern, severity in note_patterns:
            for match in re.finditer(pattern, text):
                ...
                # Splitting targets
                raw_targets = re.split(
                    r",|\sand\s|\sor\s|&", target_text, flags=re.IGNORECASE
                )
        ...
        logger.info(
            f"LLM extraction returned {len(merged_llm_rules)} AI-generated rules."
        )
    ...
    def _extract_power(
        ...
        psu_pattern = re.compile(
            r"(\d{3,4})\s*W\s+(?:Flex\s+Slot|Power\s+Supply)", re.IGNORECASE
        )
        seen_watts = set()
    ...
    COMPONENT_CATEGORY_MAP = [
        ...
        (
            ["NS204", "Boot Optimized", "Boot Device", "MicroServer Boot"],
            "Storage",
            "Boot Device",
        ),
        # Storage - Drives
        ...
        # Networking - Optics
        (
            ["Optic", "Transceiver", "SFP", "QSFP", "OSFP", "DAC Cable", "AOC"],
            "Networking",
            "Optics",
        ),
        # Networking - Switches (for Synergy-type)
        (
            ["Switch", "Interconnect", "Virtual Connect", "Fabric Module"],
            "Networking",
            "Switch/Interconnect",
        ),
        # GPU / Accelerators
        ...
        # Power
        (
            ["Power Supply", "PSU", "Flex Slot", "Power Cable", "PDU", "UPS"],
            "Power",
            "PSU",
        ),
        # Power - DC
        ...
        # Thermal
        (
            ["Heatsink", "Heat Sink", "Cooling", "Cold Plate", "DLC", "Direct Liquid"],
            "Thermal",
            "Heatsink",
        ),
        ...
        (
            ["Rail", "Rack Rail", "Friction Rail", "Easy Install Rail", "Slide Rail"],
            "Chassis",
            "Rail Kit",
        ),
        (
            ["Blank", "Filler", "DIMM Blank", "Bay Blank", "Drive Blank"],
            "Chassis",
            "Blank/Filler",
        ),
        ...
        # Infrastructure
        (
            ["Riser", "Riser Kit", "Riser Card", "Secondary Riser"],
            "Infrastructure",
            "Riser",
        ),
        (
        ...
        (
            ["Enablement", "Enable Kit", "Bay Enable", "Cage Enable", "Midplane"],
            "Infrastructure",
            "Enablement Kit",
        ),
        (
            ["Serial Port", "DisplayPort", "VGA", "USB", "I/O Module"],
            "Infrastructure",
            "I/O Port",
        ),
        (
            ["Optical Drive", "DVD", "DVD-ROM", "DVD-RW", "Media Bay"],
            "Infrastructure",
            "Optical Drive",
        ),
        # Security
        ...
        # Management
        (
            ["iLO", "Lights Out", "Management", "Compute Ops", "Insight", "OneView"],
            "Management",
            "Management Software",
        ),
    ...
    def _process_structured_components(
        ...
        for data in structured_components:
            ...
            if subcat and subcat != "General":
                limit_match = re.search(
                    r"Maximum\s+(\d+)\s+" + re.escape(subcat), desc, re.IGNORECASE
                )
                if not limit_match:
                    # Also try with category name
                    limit_match = re.search(
                        r"Maximum\s+(?:quantity\s+)?(\d+)", desc, re.IGNORECASE
                    )
                if limit_match:
            ...
            if qty > 0:
                attributes.append(
                    EngineeringAttribute(name="Default Quantity", value=qty)
                )

            weight = self.COMPONENT_WEIGHT_MAP.get(cat, 10)
            attributes.append(
                EngineeringAttribute(name="Component Weight", value=weight)
            )
            ...
            if cat == "CPU":
                ...
                if core_match:
                    attributes.append(
                        EngineeringAttribute(
                            name="Cores", value=int(core_match.group(1))
                        )
                    )
                if ghz_match:
                    attributes.append(
                ...
                if tdp_match:
                    attributes.append(
                        EngineeringAttribute(
                            name="TDP", value=int(tdp_match.group(1)), unit="W"
                        )
                    )
            ...
            if cat == "Memory":
                ...
                if cap_match:
                    attributes.append(
                        EngineeringAttribute(
                            name="Capacity", value=int(cap_match.group(1)), unit="GB"
                        )
                    )
                if ddr_match:
                    capabilities.append(ddr_match.group(1).upper())
                if speed_match:
                    attributes.append(
                        EngineeringAttribute(
                            name="Speed", value=int(speed_match.group(1)), unit="MT/s"
                        )
                    )
            ...
            if role == "dependency_note":
                requires_match = re.search(
                    r"Requires\s+([A-Z0-9]{6}-[A-Z0-9]{3}|[A-Z0-9]{6,8})",
                    desc,
                    re.IGNORECASE,
                )
                incompat_match = re.search(
                    r"Cannot be selected with\s+([A-Z0-9]{6}-[A-Z0-9]{3}|[A-Z0-9]{6,8})",
                    desc,
                    re.IGNORECASE,
                )
            ...
            comp = Component(
                id=comp_id,
                title=(
                    f"{sku} - {desc[:50]}..." if len(desc) > 50 else f"{sku} - {desc}"
                )
                .rstrip(" -")
                .strip()
                if desc
    ...
    def _extract_topology(self, text: str, platform: Optional[Platform]) -> None:
        ...
        topo_pattern = re.compile(
            r"(Mezzanine\s+Slot\s+\d+).*?(Bay\s+\d+).*?(Bay\s+\d+)", re.IGNORECASE
        )
        slot_count = 0
