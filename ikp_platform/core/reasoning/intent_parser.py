...
class IntentParser:
    ...
    def parse_request(self, raw_text: str) -> CustomerRequest:
        ...
        if not parsed.get("workloads") and not parsed.get("requirements"):
            logger.info("LLM returned empty or failed. Falling back to heuristic parsing.")
    ...
    def _extract_requirements(self, text_lower: str) -> List[CustomerRequirement]:
        ...
        if any(kw in text_lower for kw in self.component_keywords["gpu"]):
            ...
            priority = "required" if any(w in text_lower for w in ["require", "must", "need", "gpu"]) else "preferred"
            reqs.append(CustomerRequirement(
                category="technical",
                name="Accelerator",
                value="GPU",
        ...
        if any(kw in text_lower for kw in self.component_keywords["nvme"]):
            ...
            is_required = any(w in text_lower for w in ["require", "must", "critical", "mandatory"])
            reqs.append(CustomerRequirement(
                category="technical",
