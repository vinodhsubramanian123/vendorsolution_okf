...
def ingest_all():
    ...
    for pdf_file in pdf_dir.glob("*.pdf"):
        ...
        if manifest.get(pdf_file.name) == checksum:
            logger.info(f"SKIPPED {pdf_file.name}: Checksum {checksum} unchanged since last ingestion.")
            continue
        ...
        try:
            ...
            logger.error(f"FAILED: Error extracting from {pdf_file.name}: {e}", exc_info=True)
