...
class TestCategoryLimits(unittest.TestCase):
    def setUp(self):
        ...
        self.platform = Platform(id="plat1")
        self.graph.add_node(self.platform)
        ...
        self.graph.add_node(self.limit)
        
        self.engine = RuleEngine(self.graph)

    def test_within_limit(self):
        # Create 1 Riser
        riser1 = Component(id="riser1", component_category="Infrastructure", component_subcategory="Riser")
        self.graph.add_node(riser1)
        
        is_valid, chain, errors = self.engine.evaluate_solution(self.platform.id, [riser1.id])
        self.assertEqual(len(errors), 0)

    def test_exceeds_limit(self):
        riser1 = Component(id="riser1", component_category="Infrastructure", component_subcategory="Riser", inclusive_qty=3)
        self.graph.add_node(riser1)
