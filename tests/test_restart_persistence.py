...
from ikp_platform.core.ontology.models import Rule, RuleSeverity, ConfidenceLevel, EngineeringRelationship, RelationshipType
...
def test_okf_persistence_roundtrip(temp_repo_dir):
    ...
    manager1 = RepoManager(project_root=str(project_root), repository_path=str(temp_repo_dir))
    ...
    manager2 = RepoManager(project_root=str(project_root), repository_path=str(temp_repo_dir))
    manager2.bootstrap()
    ...
    assert stats2["total_edges"] == 1, "Failed to load edges after restart - Gap 0.1 regex issue!"
