...
class TestIngestion:
    def setup_method(self):
        ...
        self.extractor = HPEQuickSpecsAdapter(self.source)
    ...
    def test_rule_extraction(self, mock_llm_class):
        ...
        mock_instance.extract_rules.return_value = [
            {
                "rule_text": "Requires Qty 1 of 65cm Quick Disconnect Tube Set FIO Kit",
                "severity": "Error",
                "trigger_conditions": ["tube selected"],
            }
        ]
        mock_instance.critic_review_rules.return_value = [
            {
                "rule_text": "Requires Qty 1 of 65cm Quick Disconnect Tube Set FIO Kit",
                "severity": "Error",
                "trigger_conditions": ["tube selected"],
        ...
        text = (
            "- Requires Qty 1 of 65cm Quick Disconnect Tube Set FIO Kit (P62038-B21).\n"
        )
    ...
    def test_structured_components(self):
        ...
        components = self.extractor._process_structured_components(
            structured_data, self.platform
        )
...
class TestPlatformIdentityAgainstRealPDFs:
    ...
    def _identity_for(self, filename):
        source = Source(
            id="t",
            source_id=f"doc_{filename}",
            source_type=SourceType.PDF,
            status=ProcessingStatus.PENDING,
        )
        extractor = HPEQuickSpecsAdapter(source)
