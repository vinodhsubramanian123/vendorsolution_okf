...
from ikp_platform.core.ontology.models import Source, EngineeringObjectType
...
def main():
    ...
    if platforms:
        ...
        if "gen12" in platforms[0].id.lower() and "hpe" in platforms[0].id.lower():
            print(
                f"✅ PASS: Platform explicitly sandboxed to vendor/generation: {platforms[0].id}"
            )
        else:
            print(
                f"❌ FAIL: Platform ID doesn't isolate vendor/generation: {platforms[0].id}"
            )
    else:
    ...
    workloads = [
        o
        for o in objects
        if getattr(o, "type", None) == EngineeringObjectType.WORKLOAD.value
    ]
