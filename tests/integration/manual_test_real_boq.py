...
def main():
    ...
    for idx, row in df.iterrows():
        ...
        if "switch" in desc.lower() or "cable" in desc.lower():
            domain = "Networking"
        elif "storage" in desc.lower() or "hdd" in desc.lower() or "ssd" in desc.lower():
            domain = "Storage"
        ...
        graph.graph.add_node(node_id, 
            ...
            evidence=[{"confidence": "MEDIUM", "original_text_snippet": f"Parsed from BOQ: {desc}"}]
        )
    ...
    graph.graph.add_node("evil-san-array", 
        type="Component",
