...
def main():
    ...
    is_valid, chain, errors = engine.evaluate_solution(
        "hpe-alletra-storage", ["intel-xeon-gold-6430"]
    )
    if not is_valid and "CROSS-DOMAIN BLEED DETECTED" in str(errors):
        print(
            "✅ PASS: Engine instantly rejected the Compute component on the Storage platform."
        )
    else:
    ...
    is_valid, chain, errors = engine.evaluate_solution(
        "hpe-alletra-storage", ["aruba-cx-8320"]
    )
    if "CROSS-DOMAIN BLEED DETECTED" not in str(errors):
        print(
            "✅ PASS: Engine allowed Networking infrastructure as a valid bridge domain."
        )
    else:
